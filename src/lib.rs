...
/// Shared entrypoint for desktop binaries and Android GameActivity.
///
/// Bevy's Android integration invokes the generated `android_main` symbol, which
/// initializes `bevy::android::ANDROID_APP` before calling this function.
    ...
    app.add_plugins((
        ...
        DefaultPlugins
            .set(AssetPlugin {
                ...
                // Android assets are packed in the APK and cannot be watched.
                watch_for_changes_override: Some(!cfg!(target_os = "android")),
                ..default()
            })
            .set(WindowPlugin {
...
fn determine_initial_locale() -> unic_langid::LanguageIdentifier {
    // Priority: ITCHIO_OFFICIAL_LOCALE > LANG > system locale.
