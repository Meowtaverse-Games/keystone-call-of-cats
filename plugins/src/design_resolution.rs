...
use bevy_aspect_ratio_mask::{AspectRatioPlugin, Resolution, AspectRatioMask, Hud};
...
struct AutoMinConfig { min_width: f32, min_height: f32 }
...
impl Plugin for DesignResolutionPlugin {
    fn build(&self, app: &mut App) {
        app.insert_resource(Resolution { width: self.design.width, height: self.design.height })
            .insert_resource(AspectRatioMask { color: self.mask_color })
            .insert_resource(AutoMinConfig { min_width: self.min_width, min_height: self.min_height })
